...
def FirefoxMobile(Headless=Headless, Os=2):
    MOBILE_USER_AGENT = (
        f"Mozilla/5.0 (iPhone; CPU iPhone OS 14_1_{Os} like Mac OS X)"
...
def PlayQuiz2(override=None):
    ...
    for j in range(override):
        try:
            ...
            IG = search('IG:"([^"]+)"', txt)[
                1
            ]  # variable dans la page, pour calculer le offset
...
def PlayQuiz4(override=None):
    if not override:
        try:  # permet de gerer les truc de fidélité, qui sont plus long
            override = int(
                findall('rqQuestionState([\d]{1,2})"', driver.page_source)[-1]
            )
        except:
            override = 3
    ...
    try:
        for i in range(override):
            ...
            reponse = search('correctAnswer":"([^"]+)', txt)[
                1
            ]  # je suis pas sur qu'il y ait un espace
            reponse = reponse.replace(
                "\\u0027", "'"
            )  # il faut cancel l'unicode avec un double \ (on replacer les caracteres en unicode en caracteres utf-8)

            print(
                f"validation de la reponse                                     ",
                end="\r",
            )
    ...
    def dailyCards():
        try:
            for i in range(3):
                ...
                driver.find_element(
                    By.XPATH,
                    f"/html/body/div/div/div[3]/div[2]/div[1]/div[2]/div/div[{i+1}]/a/div/div[2]",
                ).click()
                sleep(1)
            ...
            LogError(
                f"erreur dans la premiere partie de AllCard (les daily card). cela arrive si on relance le proramme une deuxieme fois sur le meme compte \n {e}"
            )
...
def login():
    try:
        ...
        try:
            ...
            printf(
                f"erreur validation bouton KmsiCheckboxField. pas forcement grave {e}"
            )
        CustomSleep(5)
        try:
    ...
    try:
        try:
            MobileDriver = FirefoxMobile(
                Os=hash(_mail) % 10
            )  # change device for each account
        except Exception as e:
            sleep(30)
        ...
        def Mlogin(echec):
            try:
                ...
                if echec <= 3:
                    ...
                    LogError(
                        "recherche sur mobile impossible. On skip \n\n\n\n\n\n\n\n",
...
def LogPoint(account="unknown"):  # log des points sur discord
    ...
    alert.point(account=account.split("@")[0], points=point)
